document.write('<iframe id=CalFrame name=CalFrame frameborder=0 scrolling=no src="'+SERVERURL+'/javascript/calendar/calendar.html?2013-02-01-2" style="display:none;position:absolute;z-index:1001;background-color:#ffffff;"></iframe>');
document.onclick=hideCalendar;
var flay = 0;
function showCalendarZN(sImg,bOpenBound,sFld1,sNextP,sNextD,sStartD,sEndD,sVD,sOE,sVDE,sOT,s3F,sFld2,sCallback,sNextVDE,isRightPage){
	var fld1,fld2;
	...
	var eW=oImg.scrollWidth, eH=oImg.scrollHeight;
    ...
    body_width = isXhtml?document.documentElement.clientWidth:document.body.clientWidth;
    cf.style.left= ((body_width-(eL-sL)>=cf.width)?eL:body_width-cf.width).toString() + "px";//eL+eW-cf.width
	cf.style.display="block";
	wcf.openbound=bOpenBound;
	wcf.fld1=fld1;
	wcf.fld2=fld2;
	wcf.callback=sCallback;
	wcf.initCalendar(isRightPage);
...
function SelHotelCalMethod(event,type,comeid,outid,enddate,callb,bOpenBound){
	...
	if (type=="out"){
        if(bOpenBound == undefined){bOpenBound=false;}
	    event.cancelBubble=true;showCalendarZN(outid,bOpenBound,outid,'','',begindate,enddate,'','','','text','',comeid,callb);
	}
}
...
//WP.S = function(d,m,a){if(!a){this.f.value=this.t;$(this.f).prev().find("#Container span.sinfo").html(m.length>2?"<font color=red>"+m+"</font>":m);}else{
//	$(this.f).addClass("redborder_bg").tip({"text":"如果您需要在酒店入住28天以上，请致电：400-666-5511，我们会竭诚为您服务。",documentClick:true});		
//}}