	...
	for(var a=0;a<rNum;a++){
		...
		if(showlimite){
			...
			if(hotel.rooms&&hotel.rooms.length>3){
					html+='  <div class="room_more">';
					if(Hide){
						html+='    <a href="javascript:RefreshHotel('+hotel.zid+',false,'+showall+')" class="r_more">所有房型('+rNum+')</a>';
					}else{
						html+='    <a href="javascript:RefreshHotel('+hotel.zid+',true,'+showall+')" class="r_more1">部分房型('+rNum+')</a>';
					}				
					html+='  </div>';
			}
		}
